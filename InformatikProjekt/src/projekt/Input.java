...
public class Input {
	...
	public int scannerInt(String frage, int min, int max) { //method for every decision we will give. You can see an example of how I used this method in the class SchwierigCharakter and Player
        ...
        while (nummer < min || nummer > max) { //repeat while nummer is out of range
            
        	if (scanner.hasNextInt()) { //if the next input is an int, then nummer changes as that new int
        		
        		nummer = scanner.nextInt(); //a new value for nummer will be asked, this will always happen, at least once
        		
        	} else { //if the nummer asked the line above wasn't a number, then...
        		
        		System.out.println("Bitte, wählen Sie eine mögliche Antwort."); //...this will be printed, and the loop goes back to ask you for the answer
    ...
    public void closeScanner() { //method to close the scanner, VERY IMPORTANT!
        scanner.close();	//Close the scanner for every decision. If you forget to close it, then nummer will remain as what
    }						//the last value was, and be used the next time the method scannerInt() is used.
